    ...
    <div id="google-oauth" class="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden scroll-mt-6">
        ...
        <div class="p-6 sm:p-8 bg-gradient-to-r from-stone-900 to-[#1e2307] text-white flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-stone-300">Allow public visitors and guests to sign in with Google to view and track their complete stay and booking history.</p>
                </div>
            </div>
        ...
        <div class="p-6 sm:p-8 space-y-8">
            ...
            <div class="bg-blue-50/90 rounded-2xl p-6 border border-blue-200 text-xs text-blue-950 space-y-4">
                ...
                <!-- Mismatch Troubleshooting Box -->
                <div class="p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-[11px] text-amber-900 space-y-1.5">
                    <div class="font-bold flex items-center gap-1 text-amber-800">
                        <span class="material-symbols-outlined text-sm">lightbulb</span>
                        <span>Fixing "Error 400: redirect_uri_mismatch":</span>
                    </div>
                    <p>In your <a href="https://console.cloud.google.com/apis/credentials" target="_blank" class="underline font-semibold text-amber-950">Google Cloud Console ➔ Credentials</a>, edit your OAuth 2.0 Client ID and ensure the following exact URLs are added under <strong>Authorized redirect URIs</strong>:</p>
                    <div class="space-y-1 pt-1 font-mono text-[10px]">
                        <div class="flex items-center justify-between bg-white p-1.5 rounded border border-amber-300">
                            <span><?= e(BASE_URL) ?>/api/guest-google-callback.php</span>
                            <button type="button" onclick="copyCustomText('<?= e(BASE_URL) ?>/api/guest-google-callback.php', this)" class="text-blue-700 hover:text-blue-900 font-bold">Copy</button>
                        </div>
                        <?php if (strpos(BASE_URL, 'localhost') !== false): ?>
                        <div class="flex items-center justify-between bg-white p-1.5 rounded border border-amber-300">
                            <span><?= str_replace('localhost', '127.0.0.1', BASE_URL) ?>/api/guest-google-callback.php</span>
                            <button type="button" onclick="copyCustomText('<?= str_replace('localhost', '127.0.0.1', BASE_URL) ?>/api/guest-google-callback.php', this)" class="text-blue-700 hover:text-blue-900 font-bold">Copy</button>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <!-- Configuration Form -->
            <form action="<?= BASE_URL ?>/admin/users.php" method="POST" class="space-y-6">
                <input type="hidden" name="csrf_token" value="<?= Auth::generateCsrf() ?>">
                ...
                <div class="flex items-center justify-between p-4 bg-stone-50 rounded-xl border border-stone-200">
                    <div>
                        <label for="google_oauth_enabled" class="font-bold text-sm text-onyx-charcoal cursor-pointer">Enable Google Sign-in for Guest Portal</label>
                        <p class="text-xs text-stone-500">When enabled, visitors can click "Sign in with Google" on the Find Booking page (<a href="<?= BASE_URL ?>/my-booking.php" target="_blank" class="underline font-semibold text-[#343c0a]">my-booking.php</a>) to track their full reservation history.</p>
                    </div>
                ...
                <div class="pt-4 border-t border-stone-200 flex justify-end">
                    <button type="submit" class="bg-[#343c0a] hover:bg-deep-olive text-white px-7 py-3 rounded-lg text-xs font-bold tracking-wide transition shadow-sm hover:shadow flex items-center gap-2 cursor-pointer">
                        <span class="material-symbols-outlined text-base">save</span>
                        <span>Save Google OAuth Settings</span>
                    </button>
                </div>
            </form>
